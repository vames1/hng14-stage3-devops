...
class AuditLogger:
    ...
    def log_ban(self, ip, condition, rate, baseline, duration):
        ...
        entry = (
            f"[{time.strftime('%Y-%m-%dT%H:%M:%S')}] BAN {ip} | "
    ...
    def log_unban(self, ip, duration, reason):
        ...
        entry = (
            f"[{time.strftime('%Y-%m-%dT%H:%M:%S')}] UNBAN {ip} | "
            f"reason={reason} | "
            f"duration={duration_str}"
        )
        self._write(entry)
    ...
    def log_baseline(self, mean, stddev, samples):
        entry = (
            f"[{time.strftime('%Y-%m-%dT%H:%M:%S')}] BASELINE_RECALC | "
            f"mean={mean:.4f} | "
            f"stddev={stddev:.4f} | "
            f"samples={samples}"
        )
...
def main():
    ...
    # Start unbanner background thread
    ...
    for line in monitor.tail():
        ...
        # Get current rates and baseline
